...
$sqlUsuario = "SELECT id_empleado, imagen, nombre, apellido, dni, fecha_registro, direccion, email, celular, estado FROM empleados WHERE id_empleado = ?";
...
$sqlTickets = "SELECT COUNT(*) as total_tickets, SUM(total_venta) as total_ventas 
               FROM ticket_ventas WHERE id_empleado = ?";
$stmtTickets = $conexion->prepare($sqlTickets);
$stmtTickets->bind_param("i", $_SESSION['id_empleado']);
$stmtTickets->execute();
    ...
    <div class="container mt-0">
        <div class="row justify-content-center">
            <div class="col-md-6 text-center">
                <h2 class="card-title mb-0"><?= htmlspecialchars($usuario['nombre']). ' '. $usuario['apellido']  ?></h2>
                <div class="user-details text-dark">
                    <p class="mb-1">
                        <i class="fas fa-id-card me-2"></i>DNI: <?= $usuario['dni'] ?>
                    </p>
                    <p class="mb-0">
